...
const Products = () => {
    ...
    const getTotal = async () => {
        try {
            const { data } = await axios.get("http://server:3000/api/v1/product/product-count")
            setTotal(data?.total)
        } catch (error) {
            console.log(error)
        }
    ...
    const showMore = async () => {
        try {
            setLoading(true)
            const { data } = await axios.get(`http://server:3000/api/v1/product/product-list/${page}`)
            setLoading(false)
    ...
    const filterProduct = async () => {
        try {
            const { data } = await axios.post("http://server:3000/api/v1/product/product-filter", { checked })
            setProducts(data?.products)
        } catch (error) {
            console.log(error)
        }
