...
const ProductDesc = () => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                setLoading(true);
                const { data } = await axios.get(`http://16.171.24.108:3100/api/v1/product/get-product/${slug}`);
                if (data?.success) {
    ...
    const getSimilarProducts = async (pid, cid) => {
        try {
            const { data } = await axios.get(`http://16.171.24.108:3100/api/v1/product/related-products/${pid}/${cid}`);
