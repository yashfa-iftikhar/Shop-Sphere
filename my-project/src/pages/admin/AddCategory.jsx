...
const AddCategory = () => {
  ...
  const handleCategorySubmit = async (formData) => {
    try {
      ...
      const { data } = await axios.post(
        'http://server:3000/api/v1/category/create-category',
        formData,
        {
          headers: {
  ...
  const handleEdit = async (formData) => {
    try {
      const { data } = await axios.put(
        `http://server:3000/api/v1/category/update-category/${selectedCategory._id}`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete(`http://server:3000/api/v1/category/delete-category/${id}`);
      if (data?.success) {
