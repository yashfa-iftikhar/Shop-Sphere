...
const AddCategory = () => {
  ...
  const handleCategorySubmit = async (formData) => {
    try {
      ...
      const { data } = await axios.post(
        'http://16.171.24.108:3100/api/v1/category/create-category',
        formData,
        {
          headers: {
  ...
  const handleEdit = async (formData) => {
    try {
      const { data } = await axios.put(
        `http://16.171.24.108:3100/api/v1/category/update-category/${selectedCategory._id}`,
        formData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete(`http://16.171.24.108:3100/api/v1/category/delete-category/${id}`);
      if (data?.success) {
                ...
                {loading ? (
                  ...
                  filteredCategories.map((category, index) => (
                    <tr key={category._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <img
                          src={`http://16.171.24.108:3100/api/v1/category/category-photo/${category._id}`}
                          alt={category.name}
