...
const OrdersAdmin = () => {
  ...
  const getBuyerDetails = async (buyerId) => {
    try {
      const { data } = await axios.get(`http://16.171.24.108:3100/api/v1/auth/user/${buyerId}`)
      console.log(data)
      if (data?.success) {
  ...
  const getAllOrders = async () => {
    try {
      setLoading(true)
      const { data } = await axios.get('http://16.171.24.108:3100/api/v1/order/get-all-orders')
      console.log(data)
      if (data?.success) {
  ...
  const handleUpdateOrder = async (orderId, updatedData) => {
    try {
      const { data } = await axios.put(`http://16.171.24.108:3100/api/v1/order/update-order-status/${orderId}`, updatedData)
      if (data?.success) {
