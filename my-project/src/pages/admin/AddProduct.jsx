...
const AddProduct = () => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get('http://16.171.24.108:3100/api/v1/product/get-product');
      console.log(data);
      if (data?.success) {
  ...
  const handleProductSubmit = async (productData) => {
    try {
      const { data } = await axios.post(
        'http://16.171.24.108:3100/api/v1/product/create-product',
        productData,
        {
          headers: {
  ...
  const handleProductUpdate = async (productData) => {
    try {
      const { data } = await axios.put(
        `http://16.171.24.108:3100/api/v1/product/update-product/${selectedProduct._id}`,
        productData,
        {
          headers: {
            'Content-Type': 'multipart/form-data',
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (confirmDelete) {
        const { data } = await axios.delete(`http://16.171.24.108:3100/api/v1/product/delete-product/${id}`);
        if (data.success) {
  ...
  const handleView = async (product) => {
    try {
      const { data } = await axios.get(`http://16.171.24.108:3100/api/v1/product/get-product/${product.slug}`);
      if (data?.success) {
